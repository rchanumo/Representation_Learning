...
'''
Code for scraping icd produre hierarchy
Modified version of pip package icdcms 
'''
__base_url = 'http://www.icd9data.com'
__root_resource = '2015/Volume3/default.htm'
__lookup_table = dict()
...
class Node:
    ...
    def alt_code(self):
        if '-' in self.code or len(self.code) < 3:
            ...
            return '%s.%s' % (self.code[0:2], self.code[2:])
...
def _scrape_nodes_from_page(parent_node: Node, page: BSoup) -> List[Node]:
    el = page.find(attrs={'class': 'codeList'})
...
def _load():
    path = Path('./Data/icd_data/hierarchy_pc.pickle')
    if path.exists():
...
def scrape():
    ...
    index = root_html.find('ul', {'class': 'codeList'})
    top_level = [c for c in index.contents]
    ...
    icd9_codes = pd.read_excel('./Data/CMS32_DESC_LONG_SHORT_SG.xlsx', dtype=str)
    for code in icd9_codes.to_records():
        diag_code = code['PROCEDURE CODE']
    ...
    # pdb.set_trace()
    pickle.dump(root_node, Path('./Data/icd_data/hierarchy_pc.pickle').open('wb'))
